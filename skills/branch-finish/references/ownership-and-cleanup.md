...
Emitted by `git-worktree-isolation` and consumed verbatim. This skill does not define a seventh and
...
| `SKILL_OWNED` | `git-worktree-isolation` created it during this invocation, with permission, and said so in its handoff. | Permitted, subject to the state checks below. |
...
- temporary metadata this skill or `git-worktree-isolation` created for this invocation.
