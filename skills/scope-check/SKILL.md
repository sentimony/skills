---
name: scope-check
description: You MUST use this when a request needs design decisions before code — new features, product or UX behavior, architecture changes, unclear success criteria, or two materially different approaches. It starts by classifying scope, so explicitly specified mechanical refactors, localized fixes with known expected behavior, and single-outcome config changes route straight to implementation instead of a design cycle.
metadata:
...
- "skip brainstorming", "just do it", "no spec" → Route A is permitted despite doubt, but name in
  one sentence the risk this instruction takes off your hands.
- "brainstorm this properly", "grill me", "full cycle" → Route C regardless of classification.
...
- No full design cycle, no design approval loop, no mandatory `writing-plans` handoff.
...
6. **Write the design doc** to `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`, commit it.
...
9. **Terminal state: invoke `writing-plans`.** Do NOT invoke any other skill from here.
...
- Route A stated the repeated target contract and the done criterion; Route C ended with an approved
  spec and a handoff to `writing-plans`.
